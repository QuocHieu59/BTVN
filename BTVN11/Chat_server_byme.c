...
void MSG(int clients, char buf[])
{
    ...
    for (int i = 0; i < num_clientnumber; i++)
    {
        if (client_array[i].Num == clients)
            a = i;
    }
    char mess[50];
    char *STA;
    STA = strtok(buf, s);
    sprintf(mess, "%s: %s\n", client_array[a].Name, STA);
    printf("mess: %s", mess);
    for (int i = 0; i < num_clientnumber; i++)
    {
        if (client[i] != clients)
            send(client[i], mess, sizeof(mess), 0);
    }
    send(clients, OK, sizeof(OK), 0);
}

void PMSG(int clients, char buf[])
{
    char ID1[5];
    char Name[11];
    char test[5];
    int check = sscanf(buf, "%s %s %s", ID1, Name, test);
    int a;
    for (int i = 0; i < num_clientnumber; i++)
    {
        if (client_array[i].Num == clients)
    ...
    token = strtok(buf, s);
    token = strtok(buf, s);
    sprintf(mess, "%s: %s", client_array[a].Name, token);
    for (int i = 0; i < num_clientnumber; i++)
    {
        if (strcmp(Name, client_array[i].Name) == 0)
        {
            send(client_array[i].Num, mess, strlen(mess), 0);
            send(clients, OK, strlen(OK), 0);
            return;
        }
    }
    send(clients, unknow, strlen(unknow), 0);
}
